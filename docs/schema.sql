...
-- Vue : ressources publiées avec leur catégorie et auteur
CREATE OR REPLACE VIEW v_ressources_publiques AS
SELECT
    r.id,
    r.titre,
    r.description,
...
WHERE r.statut     = 'publie'
  AND r.visibilite = 'publique'
  AND u.est_actif  = 1;

-- Vue : statistiques globales
CREATE OR REPLACE VIEW v_statistiques AS
SELECT
    (SELECT COUNT(*) FROM ressources  WHERE statut = 'publie')          AS total_ressources_publiees,
    (SELECT COUNT(*) FROM ressources  WHERE statut = 'en_attente')      AS total_ressources_en_attente,
    (SELECT COUNT(*) FROM utilisateurs WHERE role = 'citoyen')          AS total_citoyens,
    (SELECT COUNT(*) FROM utilisateurs WHERE est_actif = 1)             AS total_utilisateurs_actifs,
    (SELECT SUM(vues)    FROM ressources)                                AS total_vues,
    (SELECT SUM(partages) FROM ressources)                               AS total_partages,
    (SELECT COUNT(*) FROM commentaires WHERE statut = 'approuve')       AS total_commentaires,
    (SELECT COUNT(*) FROM commentaires WHERE statut = 'en_attente')     AS total_commentaires_en_attente,
    (SELECT COUNT(*) FROM favoris)                                       AS total_favoris,
    (SELECT COUNT(*) FROM exploitations)                                 AS total_exploitations;

-- Vue : tableau de bord progression d'un utilisateur
-- Usage : SELECT * FROM v_progression_utilisateur WHERE utilisateur_id = ?
CREATE OR REPLACE VIEW v_progression_utilisateur AS
SELECT
    u.id            AS utilisateur_id,
    u.nom_complet,
    COUNT(DISTINCT f.ressource_id)  AS nb_favoris,
    COUNT(DISTINCT e.ressource_id)  AS nb_exploites,
    COUNT(DISTINCT s.ressource_id)  AS nb_sauvegardes,
    COUNT(DISTINCT r.id)            AS nb_ressources_creees
FROM utilisateurs u
LEFT JOIN favoris       f ON f.utilisateur_id = u.id
LEFT JOIN exploitations e ON e.utilisateur_id = u.id
LEFT JOIN sauvegardes   s ON s.utilisateur_id = u.id
LEFT JOIN ressources    r ON r.auteur_id      = u.id
GROUP BY u.id, u.nom_complet;

-- Vue : top ressources par vues
CREATE OR REPLACE VIEW v_top_ressources AS
SELECT
    r.id,
    r.titre,
    r.type,
    ...
    r.partages,
    c.nom AS categorie,
    (SELECT COUNT(*) FROM favoris      WHERE ressource_id = r.id) AS nb_favoris,
    (SELECT COUNT(*) FROM exploitations WHERE ressource_id = r.id) AS nb_exploitations,
    (SELECT COUNT(*) FROM commentaires WHERE ressource_id = r.id AND statut = 'approuve') AS nb_commentaires
FROM ressources r
JOIN categories c ON c.id = r.categorie_id
WHERE r.statut = 'publie' AND r.visibilite = 'publique'
ORDER BY r.vues DESC;
